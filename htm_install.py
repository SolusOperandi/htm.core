...
def main():
    ...
    if not check_cmake_version(min_cmake_version):
        ...
        subprocess.run(
            [sys.executable, "-m", "pip", "install", f"cmake>={min_cmake_version}"],
            check=True,
        )
    ...
    if not (
        os.path.exists(os.path.join(htm_core_lib_path, "htm_core.lib"))
        or os.path.exists(os.path.join(htm_core_lib_path, "libhtm_core.a"))
    ):

        # Build the C++ components with CMake
        print("Building C++ components...")
        shutil.rmtree(
            "build/cmake", ignore_errors=True
        )  # Clear cache, Ignore errors if the directory doesn't exist
        shutil.rmtree(
            "dist", ignore_errors=True
        )  # Clear wheels, Ignore errors if the directory doesn't exist
    ...
    if wheel_file is None:
        print(
            f"Error: Could not find the wheel we just created in the 'dist' directory."
        )
        sys.exit(1)
...
def find_wheel_file(project_version):
    ...
    if os.path.exists("dist"):
        wheel_files = [
            f for f in os.listdir("dist") if f.startswith("htm-") and f.endswith(".whl")
        ]
